...
#### Anammox genes (hszA and hzo)

hzsA:
```{bash}
#use this directory
cd kbase/forDIAMOND/forHZSA

#make ".fa" database file machine readable
/programs/diamond-0.9.24/diamond makedb --in hzsA.fas -d hzsA
...
do
    results="$(basename $file .fastq).out"
    /programs/diamond-0.9.24/diamond blastx -d  hzsA.dmnd -q ${file} -p 64 -e 1e-3 -k 1 -a ./$results
done;
##type ctrlX to exit, Y for yes to save, name "diamond.sh"
...
```{bash}
#use this directory
cd kbase/forDIAMOND/forHZO

#make ".fa" database file machine readable
/programs/diamond-0.9.24/diamond makedb --in hzo.fas -d hzo
...
do
    results="$(basename $file .fastq).out"
    /programs/diamond-0.9.24/diamond blastx -d  hzo.dmnd -q ${file} -p 64 -e 1e-3 -k 1 -a ./$results
done;
##type ctrlX to exit, Y for yes to save, name "diamond.sh"
