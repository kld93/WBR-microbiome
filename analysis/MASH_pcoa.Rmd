...
rownames(dist_mat_clean) <- c("10U", "11M", "12U", "7M", "8U", "9M")
colnames(dist_mat_clean) <- c("10U", "11M", "12U", "7M", "8U", "9M")

distance <- dist_mat_clean %>% 
  as_tibble() %>% 
  mutate("sample_id" = c("10U", "11M", "12U", "7M", "8U", "9M")) %>% 
  select(c("sample_id", "10U", "11M", "12U", "7M", "8U", "9M"))
...
adonis2(all_dist ~ treatment, data = meta_distance) #returns p-value of 0.1 so the groups are not significantly different
```
