...
forward_reads_18 <- sort(list.files("E:/RNA_kld/18", pattern = "_R1.fastq.gz", full.names = TRUE)) # 18 reseq
forward_reads_18
...
reverse_reads_18 <- sort(list.files("E:/RNA_kld/18", pattern = "_R2.fastq.gz", full.names = TRUE)) # 18 reseq
reverse_reads_18
...
filt_fwd_18 <- "E:/RNA_kld/trimmed_filtered/6030_KD18-reseq_R1_trimmed_filtered.fastq" # 18 reseq
filt_rev_18 <- "E:/RNA_kld/trimmed_filtered/6030_KD18-reseq_R2_trimmed_filtered.fastq" # 18 reseq
```
...
filtered_out_18 <- filterAndTrim(fwd = forward_reads_18, 
                              filt = basename(filt_fwd_18), 
                              rev = reverse_reads_18, 
                              filt.rev = basename(filt_rev_18),
                              trimLeft = c(FWD_primer_length, REV_primer_length), 
                              maxN = 0,
                              multithread = FALSE)
...
# add the resequenced 18 to the rest
filtered_full <- rbind(filtered_out, filtered_out_18)
...
```{r}
merged_amplicons <- mergePairs(dadaF = dada_fwd, derepF = filtered_fwd_reads_trimLeft, 
                               dadaR = dada_rev, derepR = filtered_rev_reads_trimLeft, 
                               verbose = TRUE)
```
