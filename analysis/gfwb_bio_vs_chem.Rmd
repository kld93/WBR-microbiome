...
tidyset <- rbind(mean_controlrun, mean_Prun, mean_NPrun) %>% 
  ...
  mutate(pct_P_removal = ((initial_p - mean_P)/initial_p) * 100)
...
C_CNS_corr <- tidyset %>% 
  filter(treatment == "Control") %>%
  filter(reactor == "CNS") %>% 
  select(c(mean_Al, mean_Ca, mean_Fe, mean_Mg, mean_Mn, mean_P, mean_S, mean_Cl, 
           mean_NO2, mean_NO3, mean_PO4, mean_SO4, initial_p, P_change, pct_P_removal)) %>% 
...
pairs.panels(C_CNS_corr, method = "spearman", stars = TRUE)
pairs.panels(C_DRW_corr, method = "spearman", stars = TRUE)
pairs.panels(P_CNS_corr, method = "spearman", stars = TRUE)
pairs.panels(P_DRW_corr, method = "spearman", stars = TRUE)
pairs.panels(N_CNS_corr, method = "spearman", stars = TRUE)
pairs.panels(N_DRW_corr, method = "spearman", stars = TRUE)
