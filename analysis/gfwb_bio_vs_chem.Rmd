...
write.csv(tidyset, file = "data/RNA/nutrient_conc.csv")
...
control_plot <- tidyset %>%
  ...
  theme_classic() +
  labs(color = "Reactor:", title = "Control") +
  ylim(c(0,0.3)) +
  xlab("Hours After Start") +
...
NP_plot <- tidyset %>%
  ...
  scale_x_continuous(breaks = breaks_N, labels = timepoints_N) +
  #facet_wrap(~treatment, ncol = 1) +
  geom_vline(xintercept = c(792, 840), color="#BB0000", linetype="dashed") +
  theme_classic() +
  labs(color = "Reactor:", title = "P+N") +
  ylim(c(0,0.3)) +
  xlab("Hours After Start") +
  ylab ("P Concentration (mg/L)") +
  theme(legend.position = "bottom",
